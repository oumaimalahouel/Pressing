...
function rechercheClients(){


        var cleRech=document.getElementById("cleRechClient").value;
		document.getElementById("contenu-bl-list").innerHTML="";
	    var datastring='cleRech='+cleRech;
        $.getJSON("php/3_GestionClients/RechercherClients.php",datastring,
...
function ligneClient(id_client,totalComm,prenom,nom,tel,adresse,email,nbrComm,num_ligne,background_ligne)
{
                        ...
                        else
						{
                          ...
                          $('#contenu-bl-list').append('<a href="javascript:detailClient'+num_ligne+'('+id_client+');" style="color:#000;"><div class="w-section entete-entrer cat-entete-entrer liste-entrer liste-entrer2" id="itemClt'+id_client+'" style="background:#'+background_ligne+'"><div class="txt-id">'+id_client+'</div><span class="txt-id txt-client">'+nom+' '+prenom+'</span><span class="txt-id txt-client">'+tel+'</span><span  style="margin-left:100px;">'+totalCommande.toFixed(3)+ ' DT </span></div></div></a>');
